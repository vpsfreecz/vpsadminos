...
let
  ...
  machineConfigFile = repoCfg: pkgs.writeText "machine-config.json" (machineJsonConfig repoCfg);

  stateDir = repoName: "/var/lib/build-vpsadminos-repository/${repoName}";

  osvmScript =
    repoName: repoCfg:
    pkgs.writeText "build-vpsadminos-repository-${repoName}.rb" ''
      require 'fileutils'
      require 'json'
  ...
  enabledTimers =
    repos: filterAttrs (repoName: repoCfg: repoCfg.enable && repoCfg.systemd.timer.enable) repos;
