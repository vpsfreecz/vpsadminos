...
sub showHelp {
    print <<'EOF';
Usage: vpsadminos-generate-config [--root PATH] [--dir PATH] [--force]
                                  [--flake-url URL] [--nixpkgs-flake-url URL]
                                  [--hostname NAME] [--no-flake]
                                  [--no-filesystems] [--show-hardware-config]

Generate vpsAdminOS hardware and system configuration files.
EOF
}
...
sub defaultHostName {
    my $hostName = read_file("/proc/sys/kernel/hostname", err_mode => "quiet");
    chomp $hostName if defined $hostName;
    return $hostName || "vpsadminos";
}

# Process the command line.
my $outDir = "/etc/vpsadminos";
my $rootDir = ""; # = /
my $force = 0;
my $noFilesystems = 0;
my $showHardwareConfig = 0;
my $generateFlake = 1;
my $flakeUrl = defined $ENV{"VPSADMINOS_FLAKE_URL"}
    ? $ENV{"VPSADMINOS_FLAKE_URL"}
    : "github:vpsfreecz/vpsadminos";
my $nixpkgsFlakeUrl = $ENV{"VPSADMINOS_NIXPKGS_FLAKE_URL"};
my $hostName = $ENV{"VPSADMINOS_HOSTNAME"};
...
for (my $n = 0; $n < scalar @ARGV; $n++) {
    ...
    elsif ($arg eq "--flake-url") {
        $n++;
        $flakeUrl = $ARGV[$n];
        die "$0: ‘--flake-url’ requires an argument\n" unless defined $flakeUrl;
    }
    elsif ($arg eq "--nixpkgs-flake-url") {
        $n++;
        $nixpkgsFlakeUrl = $ARGV[$n];
        die "$0: ‘--nixpkgs-flake-url’ requires an argument\n" unless defined $nixpkgsFlakeUrl;
    }
    elsif ($arg eq "--hostname") {
        $n++;
        $hostName = $ARGV[$n];
        die "$0: ‘--hostname’ requires an argument\n" unless defined $hostName;
    }
    elsif ($arg eq "--no-flake") {
        $generateFlake = 0;
    }
...
$hostName = defaultHostName() if !defined $hostName || $hostName eq "";
...
# Pull in OS configuration for containers.
...
my $hwConfig = <<EOF;
# Do not modify this file!  It was generated by ‘vpsadminos-generate-config’
# and may be overwritten by future invocations.  Please make changes
# to /etc/vpsadminos/configuration.nix instead.
...
$bootLoaderConfig
  ...
  # This value determines the vpsAdminOS release with which your system is to be
...
EOF
    ...
    if ($generateFlake) {
        ...
        if ($force || ! -e $fn) {
            ...
            my $nixpkgsInput = "";
            if (defined $nixpkgsFlakeUrl && $nixpkgsFlakeUrl ne "") {
                $nixpkgsInput = <<EOF;
    nixpkgs.url = ${\(toNixString($nixpkgsFlakeUrl))};
...
EOF
            ...
            write_file($fn, <<EOF);
{
  description = "vpsAdminOS system configuration";

  inputs = {
    vpsadminos.url = ${\(toNixString($flakeUrl))};
$nixpkgsInput  };

  outputs = { self, vpsadminos, ... }: {
    vpsadminosConfigurations.${\(toNixString($hostName))} = vpsadminos.lib.vpsadminosSystem {
