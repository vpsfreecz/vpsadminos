...
while [ "$#" -gt 0 ]; do
    ...
    case "$i" in
      ...
      --install-grub)
        ...
        export OS_INSTALL_BOOTLOADER=1
        ;;
      --install-bootloader)
        export OS_INSTALL_BOOTLOADER=1
        ;;
...
# executed, so it's safe to run os-rebuild against a potentially
# untrusted tree.
canRun=
...
if [ -n "$upgrade" -a -z "$_OS_REBUILD_REEXEC" ]; then
    ...
    # ".update-on-os-rebuild", update them as well.
    for channelpath in /nix/var/nix/profiles/per-user/root/channels/*; do
        if [ -e "$channelpath/.update-on-os-rebuild" ]; then
...
# important, because vpsAdminOS defaults the architecture of the rebuilt
# system to the architecture of the nix-* binaries used.  So if on an
# amd64 system the user has an i686 Nix package in her PATH, then we
# would silently downgrade the whole system to be i686 NixOS on the
# next reboot.
if [ -z "$_OS_REBUILD_REEXEC" ]; then
...
tmpDir=$(mktemp -t -d os-rebuild.XXXXXX)
