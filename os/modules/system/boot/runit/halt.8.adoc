...
*HALT_HOOK*:: pre-run/pre-system
*HALT_ACTION*:: poweroff/reboot/kexec
*HALT_REASON*:: halt reason
*HALT_FORCE*:: *1* if `--force` is used, *0* otherwise
*HALT_KEXEC*:: *1* if `--kexec` is allowed, *0* otherwise
...
*-k*, *--[no-]kexec*::
  Enable or disable reboot using kexec. Note that target kernel/initrd must already
  be loaded. kexec is used by default on *reboot* if it is loaded.
