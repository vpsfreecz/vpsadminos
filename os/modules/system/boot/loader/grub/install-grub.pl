...
my $extraEntriesBeforeVpsAdminOS = get("extraEntriesBeforeVpsAdminOS") eq "true";
...
# Add default entries.
$conf .= "$extraEntries\n" if $extraEntriesBeforeVpsAdminOS;

addEntry("vpsAdminOS - Default", $defaultConfig, "--unrestricted");

$conf .= "$extraEntries\n" unless $extraEntriesBeforeVpsAdminOS;
...
foreach my $link (@links) {
    ...
    addEntry("vpsAdminOS - $entryName", $link);
}
...
sub addProfile {
    ...
    foreach my $link (@links) {
        ...
        ? readFile("$link/os-version")
        : basename((glob(dirname(Cwd::abs_path("$link/kernel")) . "/lib/modules/*"))[0]);
        addEntry("vpsAdminOS - Configuration " . nrFromGen($link) . " ($date - $version)", $link);
    }

    $conf .= "}\n" if $grubVersion == 2;
}

addProfile "/nix/var/nix/profiles/system", "vpsAdminOS - All configurations";

if ($grubVersion == 2) {
    for my $profile (glob "/nix/var/nix/profiles/system-profiles/*") {
        my $name = basename($profile);
        next unless $name =~ /^\w+$/;
        addProfile $profile, "vpsAdminOS - Profile '$name'";
    }
}
