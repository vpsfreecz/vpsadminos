...
Select work from operator instructions, canonical local Markdown, and live
local Git, source, process, and test state. The single root agent owns review,
execution, integration, and closure. There is no separate reviewer,
orchestration identity, or issue/PR control plane. Retired Forgejo and Nexus
records are historical input only and cannot select work or add acceptance
gates.

Review Git history and source directly, then record ownership decisions, exact
refs, validation results, and remaining work in the existing Markdown release
tracker.
