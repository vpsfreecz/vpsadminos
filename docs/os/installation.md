...
vpsAdminOS provides `vpsadminos-generate-config` and `vpsadminos-install` tools
...
```bash
vpsadminos-generate-config --root /mnt --hostname my-node
```

This writes `hardware-configuration.nix`, `configuration.nix` and a flake in
`/mnt/etc/vpsadminos`. In `/mnt/etc/vpsadminos/configuration.nix` make sure to
set `boot.loader.grub.device` option to point to drive where GRUB should be
installed. You can also set `boot.loader.grub.devices` to install GRUB to
multiple devices.

You can also configure your [ZFS pools](pools.md) at this point.

After adjustments of generated configs in `/mnt/etc/vpsadminos/` OS can be installed using

```bash
vpsadminos-install --root /mnt --flake /mnt/etc/vpsadminos#my-node
```
...
If you need to chroot into vpsAdminOS installation use `vpsadminos-enter` utility - if your installation
is mounted at `/mnt` you can chroot into it by running:

```bash
vpsadminos-enter --root /mnt
```
