...
The userspace builder compiles its prepared source with the vpsAdminOS target
path `/run/current-system/sw/bin/exportfs` instead of OpenZFS's host-oriented
`/usr/sbin/exportfs`. Enabling the NFS server places `nfs-utils` in that system
profile, matching the runtime contract of the normal Nix-built ZFS package and
preventing a raw development stage from consulting the build host's FHS paths.
...
The remote SSH login environment must provide Bash in `PATH`. The launcher
resolves that executable to an absolute path before calling `systemd-run`, so
the offload host does not need an FHS `/bin/bash` path.
...
Each lane launch has a unique launch ID, and its systemd wrapper atomically
records a matching terminal row only after `test-json-runner` returns. `wait`,
`watch`, and manual `collect` all require that exact row. A missing, malformed,
stale, or transport-inaccessible row is never inferred to be success. Collection
also requires exit status zero and at least one `expected_success` or
`expected_failure` test result for every lane, with no unexpected or missing
result. The per-lane evidence is retained in `lane-terminals.tsv` and included
in `summary.json`.
