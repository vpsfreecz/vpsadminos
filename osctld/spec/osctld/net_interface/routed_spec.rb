...
RSpec.describe OsCtld::NetInterface::Routed do
  ...
  it 'forwards an authenticated absent host-veth replacement' do
    routed.create(name: 'eth0', hwaddr: nil)
    routed.up('veth0')
    allow(routed).to receive(:host_veth_ifindex!)
      .with('veth1', expected_kind: 'veth')
      .and_return(11)
    allow(routed).to receive(:recorded_host_link_absent?)
      .with('veth0', 10)
      .and_return(true)

    routed.up('veth1', allow_absent_replacement: true)

    expect(routed.host_link_identity).to eq(['veth1', 11, nil])
  end
