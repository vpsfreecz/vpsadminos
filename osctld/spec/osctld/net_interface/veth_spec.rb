...
RSpec.describe OsCtld::NetInterface::Veth do
  ...
  it 'replaces a clean absent host veth for an authenticated restart' do
    snapshots = []
    allow(ct).to receive(:save_config) { snapshots << veth.save }
    veth.create(name: 'eth0', hwaddr: nil)
    veth.up('veth0')
    allow(veth).to receive(:host_veth_ifindex!)
      ...
      .and_return(11)
    allow(veth).to receive(:recorded_host_link_absent?)
      .with('veth0', 10)
      .and_return(true)

    veth.up('veth1', allow_absent_replacement: true)

    expect(veth.host_link_identity).to eq(['veth1', 11, nil])
  ...
  it 'rejects an absent host-veth replacement without lifecycle authorization' do
    veth.create(name: 'eth0', hwaddr: nil)
    veth.up('veth0')
    allow(veth).to receive(:host_veth_ifindex!)
      .with('veth1', expected_kind: 'veth')
  ...
  it 'rejects an authenticated replacement while the old host veth may exist' do
    veth.create(name: 'eth0', hwaddr: nil)
    veth.up('veth0')
    allow(veth).to receive(:host_veth_ifindex!)
      .with('veth1', expected_kind: 'veth')
  ...
  it 'rejects an authenticated replacement of a cleanup-tainted host veth' do
    veth.create(name: 'eth0', hwaddr: nil)
    veth.up('veth0')
  ...
  it 'rejects an authenticated replacement while the old IFB remains recorded' do
    veth.create(name: 'eth0', hwaddr: nil, max_tx: 100)
    veth.up('veth0')
    allow(veth).to receive(:host_veth_ifindex!)
      .with('veth1', expected_kind: 'veth')
  ...
  it 'releases a clean absent link only when authenticated cleanup permits it' do
    veth.create(name: 'eth0', hwaddr: nil)
    veth.up('veth0')
    allow(veth).to receive(:recorded_host_link_absent?)
      .with('veth0', 10)
    ...
    expect(
      veth.down('veth0', allow_absent_cleanup: true)
    ).to eq('veth0')
  ...
  it 'does not release an absent cleanup-tainted link' do
    ...
    veth.up('veth0')
    veth.instance_variable_set(:@host_link_tainted, true)
    allow(veth).to receive(:recorded_host_link_absent?).and_return(true)
    ...
    expect(veth.host_link_identity).to eq(['veth0', 10, nil])
    expect(veth.host_link_tainted?).to be(true)
    expect(veth).not_to have_received(:recorded_host_link_absent?)
  end
