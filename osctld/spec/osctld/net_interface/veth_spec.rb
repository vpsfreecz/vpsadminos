...
RSpec.describe OsCtld::NetInterface::Veth do
  ...
  describe 'explicit absent-link recovery' do
    before do
      veth.create(name: 'eth0', hwaddr: nil)
      veth.load(veth.save.merge('host_link' => {
        'name' => 'veth0', 'ifindex' => 10, 'ifb_ifindex' => 20,
        'boot_id' => host_boot_id, 'tainted' => true
      }))
    end

    it 'forgets tainted ownership only after checking both selectors for veth and IFB' do
      selectors = []
      links = Object.new
      links.define_singleton_method(:[]) do |key|
        selectors << key
        nil
      end
      allow(Linux::Netlink::Route::Socket).to receive(:open).and_yield(Struct.new(:link).new(links))

      veth.forget_absent_host_link!

      expect(selectors).to eq(['veth0', 10, 'ifbveth0', 20])
    ...
    ['veth0', 10, 'ifbveth0', 20].each do |replacement|
      it "preserves a replacement found by #{replacement.inspect}" do
        links = { replacement => Object.new }
        allow(Linux::Netlink::Route::Socket).to receive(:open).and_yield(Struct.new(:link).new(links))

        expect { veth.forget_absent_host_link! }.to raise_error(described_class::InvalidHostLink)
    ...
    it 'preserves the record when querying the kernel fails' do
      allow(Linux::Netlink::Route::Socket).to receive(:open).and_raise(Errno::EPERM)

      expect { veth.forget_absent_host_link! }.to raise_error(Errno::EPERM)
      expect(veth.host_link_identity).to eq(['veth0', 10, 20])
      expect(veth).not_to have_received(:delete_link_by_ifindex)
    end
  end
  ...
  it 'rejects an absent host-veth replacement without explicit recovery' do
    veth.create(name: 'eth0', hwaddr: nil)
    veth.up('veth0')
    allow(veth).to receive(:host_veth_ifindex!)
      .with('veth1', expected_kind: 'veth')
      .and_return(11)

    expect { veth.up('veth1') }.to raise_error(
      described_class::InvalidHostLink,
      /host veth identity changed/
    )

    expect(veth.host_link_identity).to eq(['veth0', 10, nil])
  end

  it 'rejects a replacement of a cleanup-tainted host veth' do
    veth.create(name: 'eth0', hwaddr: nil)
    veth.up('veth0')
    veth.instance_variable_set('@host_link_tainted', true)
    allow(veth).to receive(:host_veth_ifindex!)
      .with('veth1', expected_kind: 'veth')
      .and_return(11)
  ...
  it 'rejects a replacement while the old IFB remains recorded' do
    veth.create(name: 'eth0', hwaddr: nil, max_tx: 100)
    veth.up('veth0')
    allow(veth).to receive(:host_veth_ifindex!)
      .with('veth1', expected_kind: 'veth')
  ...
  it 'does not release an absent cleanup-tainted link' do
    veth.create(name: 'eth0', hwaddr: nil)
    veth.up('veth0')
    veth.instance_variable_set(:@host_link_tainted, true)
    allow(veth).to receive(:delete_link_by_ifindex).and_raise(
      OsCtld::Utils::Ip::LinkIdentityError,
