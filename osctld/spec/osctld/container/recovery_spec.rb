...
RSpec.describe OsCtld::Container::Recovery do
  ...
  describe 'explicit host-link acknowledgment' do
    before do
      stub_const('OsCtld::ContainerControl::Commands::State', Class.new do
        def self.run!(_ct, force:)
          Struct.new(:state, :init_pid).new(:stopped, nil)
        end
      end)
      allow(ct).to receive(:netifs).and_return({ 'eth0' => netif })
      allow(netif).to receive(:forget_absent_host_link!)
      allow(OsCtld::NetInterface).to receive(:validate_host_link_owner!)
    end

    it 'checks LXC and preserves the existing generic error before forgetting the named record' do
      events = []
      allow(ct).to receive(:state).and_return(:error)
      allow(OsCtld::ContainerControl::Commands::State).to receive(:run!)
        .with(ct, force: true).and_wrap_original do |original, *args, **kwargs|
          events << :state
          original.call(*args, **kwargs)
        end
      allow(netif).to receive(:forget_absent_host_link!) { events << :forget }

      recovery.forget_host_link('eth0')

      expect(events).to eq(%i[state forget])
      expect(ct).not_to have_received(:state=)
      expect(ct).not_to have_received(:clear_recovery_taint!)
    end

    [[:running, 123], [:starting, nil], [:stopped, 123]].each do |state, pid|
      it "refuses LXC state #{state} with init #{pid.inspect}" do
        allow(OsCtld::ContainerControl::Commands::State).to receive(:run!)
          .and_return(Struct.new(:state, :init_pid).new(state, pid))

        expect { recovery.forget_host_link('eth0') }
          .to raise_error(described_class::InvalidNetifIdentity, /stopped/)
        expect(netif).not_to have_received(:forget_absent_host_link!)
      end
    end

    it 'refuses an unknown configured interface' do
      expect { recovery.forget_host_link('eth1') }
        .to raise_error(described_class::InvalidNetifIdentity, /not found/)
      expect(netif).not_to have_received(:forget_absent_host_link!)
    end

    it 'preserves the record when LXC cannot be queried' do
      allow(OsCtld::ContainerControl::Commands::State).to receive(:run!).and_raise('LXC failed')
      expect { recovery.forget_host_link('eth0') }.to raise_error(RuntimeError, 'LXC failed')
      expect(netif).not_to have_received(:forget_absent_host_link!)
    end
  end
  ...
  it 'taints the container when direct and route-based identity cleanup fail' do
    ...
    expect(ct).to have_received(:taint_recovery!)
    expect(ct).not_to have_received(:state=)
    expect(netif).not_to have_received(:down)
  end
  ...
  it 'persists fallback cleanup and clears only recovery taint on a complete retry' do
    ...
    allow(netif).to receive(:down) do |name|
      ...
      snapshots << [current_state, tainted, current_identity.dup]
    end
    take_down_calls = 0
    ...
    allow(ct).to receive(:recovery_tainted?) { tainted }
    allow(ct).to receive(:taint_recovery!) do
      tainted = true
      ct.save_config
    end
    allow(ct).to receive(:clear_recovery_taint!) do
      tainted = false
      ct.save_config
    end
    ...
    expect(snapshots).to eq(
      [
        [:error, false, [nil, nil, nil]],
        [:error, true, [nil, nil, nil]]
      ]
    )

    # A complete recovery pass must not clear the unrelated generic error.
    expect(recovery.cleanup_or_taint).to be(true)
    expect(current_state).to eq(:error)
    expect(snapshots.last).to eq([:error, false, [nil, nil, nil]])
  end
