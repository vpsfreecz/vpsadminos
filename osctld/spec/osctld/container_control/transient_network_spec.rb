...
RSpec.describe OsCtld::ContainerControl::TransientNetwork do
  ...
  it 'does not snapshot routed gateways before the transient interfaces exist' do
    allow(OsCtld::NetConfig).to receive(:create).and_return(network)

    pending_server = described_class.new(container)
    pending_server.close

    expect(OsCtld::NetConfig).not_to have_received(:create)
  end
