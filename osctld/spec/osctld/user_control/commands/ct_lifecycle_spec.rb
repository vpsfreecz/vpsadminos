...
RSpec.describe 'authenticated container lifecycle callbacks' do
  ...
  it 'reboots and tears down exactly the authenticated run' do
    ...
    netif = instance_double('NetInterface', is_created?: true, down: nil)
    ...
    allow(ct).to receive_messages(pool:, id: 'ct1', netifs: [netif])
    ...
    expect(netif).to have_received(:down).with(nil, allow_absent_cleanup: true)
