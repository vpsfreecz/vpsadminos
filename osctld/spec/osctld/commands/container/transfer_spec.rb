...
RSpec.describe 'container transfer commands' do
  ...
  describe OsCtld::Commands::Container::Copy do
    ...
    it 'cleans up the first copy snapshot when stopping the source fails' do
      src_dataset = build_dataset('tank/ct1')
      dst_dataset = build_dataset('tank/target/ct1')
    ...
    it 'cleans up both snapshots when restarting the source fails' do
      src_dataset = build_dataset('tank/ct1')
      dst_dataset = build_dataset('tank/target/ct1')
      ct = Struct.new(:dataset, :datasets, :id, :pool, keyword_init: true) do
        def running?
      ...
      ctrc = Struct.new(:dataset, :map_mode).new(dst_dataset, 'zfs')
      builder_class = Class.new do
        def ctrc; end
      ...
      allow(builder).to receive(:create_dataset)
      allow(builder).to receive(:copy_datasets).and_return('snap1', 'snap2')
      allow(command).to receive(:call_cmd!)
        .with(OsCtld::Commands::Container::Stop, id: 'ct1', pool: 'tank')
        .and_return(status: true, output: nil)
      allow(command).to receive(:call_cmd!)
