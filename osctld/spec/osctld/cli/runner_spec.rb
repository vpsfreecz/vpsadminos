...
RSpec.describe OsCtld::Cli::Runner do
  ...
  it 'reports execution errors without misclassifying them as setup failures' do
    ret = instance_double(IO, puts: nil)
    stdout = instance_double(IO)
    stderr = instance_double(IO)
    ...
    runner_class = Class.new do
      def initialize(pool:, id:, lxc_home:, user_home:, log_file:, stdin:, stdout:, stderr:, network_socket: nil); end
    ...
    expect(ret).to have_received(:puts) do |payload|
      ...
      expect(data.fetch('message')).to eq('helper execution failed (RuntimeError)')
      expect(data.fetch('user_runner')).to be(false)
      expect(data.fetch('diagnostic')).to include('runner exploded', __FILE__)
    end
  end

  it 'reports initialization failures on the private return channel' do
    ret = instance_double(IO, puts: nil)
    allow(IO).to receive(:new).with(10).and_return(ret)
    allow(ret).to receive(:close_on_exec=)
    allow(OsCtl::Lib::Logger).to receive(:setup)
    stub_const('OsCtld::CGroup', Module.new do
      def self.init
        raise Errno::EACCES, 'private cgroup path'
      end
    end)
    allow($stdin).to receive(:readline).and_return({ return: 10 }.to_json)

    expect { with_argv { described_class.run } }.to raise_error(SystemExit)
    expect(ret).to have_received(:puts) do |payload|
      data = JSON.parse(payload)
      expect(data.fetch('message')).to eq('helper setup failed (Errno::EACCES)')
      expect(data.fetch('user_runner')).to be(true)
      expect(data.fetch('diagnostic')).to include('private cgroup path', __FILE__)
    end
  end
end
