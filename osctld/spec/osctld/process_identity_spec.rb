...
RSpec.describe OsCtld::ProcessIdentity do
  ...
  it 'loads the prototype start_time key for runtime compatibility' do
    identity = described_class.load(
      'pid' => Process.pid,
      'start_time' => described_class.read_start_time(Process.pid)
    )

    expect(identity).to be_alive
  end
end
