...
RSpec.describe OsCtld::ProcessIdentity do
  ...
  it 'normalizes a proc entry open race after process exit' do
    allow(sys).to receive(:pidfd_alive?).with(pidfd).and_return(true, false)
    allow(File).to receive(:open).with('/proc/123').and_return(proc_dir)
    allow(sys).to receive(:openat_io).with(proc_dir, 'root').and_raise(Errno::EACCES)

    expect do
      described_class.new(123, root: true)
    end.to raise_error(Errno::ESRCH, 'No such process - process 123 exited')

    expect(pidfd).to have_received(:close)
    expect(proc_dir).to have_received(:close)
  end

  it 'preserves proc entry open failures while the process is live' do
    allow(File).to receive(:open).with('/proc/123').and_return(proc_dir)
    allow(sys).to receive(:openat_io).with(proc_dir, 'root').and_raise(Errno::EACCES)
    ...
    expect do
      ...
      Errno::EACCES,
      %r{unable to open /proc/123/root through retained proc descriptor}
    )
  end
  ...
  it 'resolves relative cgroup names from the hierarchy root, not the working directory' do
    identity = described_class.allocate
    allow(identity).to receive(:read_proc_file).with('cgroup').and_return(<<~CGROUP)
      0::/osctl/pool.tank/ct.ct1/user-owned/lxc.payload.ct1
    CGROUP

    Dir.mktmpdir('osctld-cgroup-cwd') do |dir|
      Dir.chdir(dir) do
        expect(identity.in_cgroup_subtree?('osctl/pool.tank/ct.ct1/user-owned')).to be(true)
        expect(identity.in_cgroup_subtree?('osctl/pool.tank/ct.ct2/user-owned')).to be(false)
      end
    end
  end
