...
RSpec.describe OsCtld::Monitor::Process do
  ...
  it 'keeps monitoring when the reported init cannot be pinned' do
    ct = build_ct
    db = stub_const('OsCtld::DB::Containers', Class.new do
      def self.find(_id, _pool); end
    end)
    ...
    allow(ct).to receive(:set_init_pid).with(5678).and_raise(Errno::EACCES)
    ...
    expect(OsCtl::Lib::Logger).to have_received(:log).with(
      :warn,
      '[monitor] Unable to get state of container tank:ct1: Permission denied'
    )
  end
