...
module OsCtld
  class BpfFs
    ...
    # A generation switch does not run the boot-time mount setup. Bind the
    # predecessor's existing bpffs so device programs and links stay pinned;
    # mounting a fresh filesystem here would lose their userspace identities.
    def setup_host_mount
      mounted_type = mount_type(FS)
      return if mounted_type == 'bpf'

      raise "#{FS} is not a BPF filesystem" if mounted_type
      raise 'legacy BPF filesystem is not mounted' unless mount_type('/sys/fs/bpf') == 'bpf'

      FileUtils.mkdir_p(FS)
      syscmd("mount --bind /sys/fs/bpf #{FS}")
      syscmd("mount --make-rprivate #{FS}")
    end
