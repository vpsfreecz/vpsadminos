...
module OsCtld
  ...
  class ContainerControl::TransientNetwork
    ...
    def initialize(ct, net_config = nil)
      @ct = ct
    ...
    def serve(runner_identity)
      ...
      ProcessIdentity.open(state.init_pid, namespaces: %i[net user]) do |identity|
        ...
        # Routed IPv6 gateways belong to host veths created during LXC start.
        # Snapshot daemon-owned configuration only after authenticated readiness.
