...
module OsCtld
  ...
  class CpuScheduler
    ...
    #
    # If there are two packages, then containers are first started on package0
    # and then on package1. Stop goes in reverse order: package1, package0.
    #
        ...
        if sched
          ...
          pkg =
            if wanted_pkg_id
              ...
              target_pkg = package_info.values.select(&:enabled).min_by(&:id)

              if target_pkg
                log(:debug, "Priority start for #{ct.ident}, using CPU package #{target_pkg.id}")
                get_package_by_preference(target_pkg.id, daily_use)
              end
