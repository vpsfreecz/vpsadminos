    ...
    def execute
      ...
      with_claimed_lifecycle_event(
        ...
        accepted = ct.stopped(run_conf) do |teardown_owner|
          ...
          # LXC may remove a veth without invoking its down hook. Reconcile any
          # residual identity while this authenticated run still owns teardown.
          capture_teardown_failure(teardown_failures, :netifs) do
            ct.netifs.each do |netif|
              next unless netif.is_created?

              netif.down(nil, allow_absent_cleanup: true)
            end
          end
