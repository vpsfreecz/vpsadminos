...
module OsCtld
  class NetInterface::Veth < NetInterface::Base
    ...
    def up(veth, allow_absent_replacement: false)
      NetInterface.sync_host_link_registry do
        veth_name, ifindex = validate_up_callback(
          veth,
          allow_absent_replacement:
        )

        replaced_identity = exclusively do
          previous_identity = nil

          if @veth && (@veth != veth_name || @veth_ifindex != ifindex)
            unless allow_absent_replacement && replaceable_absent_host_link?
              raise InvalidHostLink,
                    "host veth identity changed from #{@veth}(#{@veth_ifindex}) " \
                    "to #{veth_name}(#{ifindex})"
            end

            previous_identity = [@veth, @veth_ifindex]
          end
        ...
        if replaced_identity
          log(
            :info,
            ct,
            "Replaced absent host veth #{replaced_identity[0]}" \
            "(#{replaced_identity[1]}) with #{veth_name}(#{ifindex})"
          )
        end
    ...
    def down(host_veth = nil, allow_absent_cleanup: false)
      NetInterface.sync_host_link_registry do
        veth_name, expected_ifindex, expected_ifb_ifindex =
          validate_down_callback(host_veth)
        return unless veth_name

        absent_cleanup = allow_absent_cleanup &&
                         !host_link_tainted? &&
                         expected_ifb_ifindex.nil? &&
                         recorded_host_link_absent?(veth_name, expected_ifindex)

        if absent_cleanup
          log(
            :info,
            ct,
            "Releasing absent host veth #{veth_name} (ifindex #{expected_ifindex})"
          )
        else
    ...
    def validate_up_callback(host_veth, allow_absent_replacement: false)
      NetInterface.sync_host_link_registry do
        ...
        inclusively do
          ...
          if @veth && (@veth != veth_name || @veth_ifindex != ifindex) &&
             !(allow_absent_replacement && replaceable_absent_host_link?)
            raise InvalidHostLink,
                  "host veth identity changed from #{@veth}(#{@veth_ifindex}) " \
                  "to #{veth_name}(#{ifindex})"
          end
    ...
    def replaceable_absent_host_link?
      return false if @host_link_tainted || @ifb_ifindex
      return false unless @veth && @veth_ifindex.is_a?(Integer) && @veth_ifindex > 0

      recorded_host_link_absent?(@veth, @veth_ifindex)
    end

    def recorded_host_link_absent?(name, ifindex)
      Linux::Netlink::Route::Socket.open do |nl|
        by_name = find_host_link(nl, name)
        by_index = find_host_link(nl, ifindex)

        by_name.nil? && by_index.nil?
      end
    end

    def find_host_link(nl, key)
      nl.link[key]
