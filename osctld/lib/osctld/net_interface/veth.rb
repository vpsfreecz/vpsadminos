...
module OsCtld
  class NetInterface::Veth < NetInterface::Base
    ...
    # Used only by the privileged stopped-container recovery operation.
    # Do not delete anything: names and indexes can belong to replacement
    # links. Query both selectors even when one of them is already absent.
    def forget_absent_host_link!
      NetInterface.sync_host_link_registry do
        veth_name, veth_index, ifb_index = host_link_identity
        unless veth_name && veth_index.is_a?(Integer) && veth_index > 0
          raise InvalidHostLink, 'no valid host-link ownership record'
        end

        Linux::Netlink::Route::Socket.open do |nl|
          links = [[veth_name, veth_index]]
          links << ["ifb#{veth_name}", ifb_index] if ifb_index
          links.each do |link_name, link_index|
            by_name = find_host_link(nl, link_name)
            by_index = find_host_link(nl, link_index)
            unless by_name.nil? && by_index.nil?
              raise InvalidHostLink,
                    "host link #{link_name.inspect} or ifindex #{link_index} is still present"
            end
          end
        end
