...
module OsCtld
  ...
  class Container::Recovery
    ...
    # Cleanup after the container or retain a separate recovery taint
    # @return [Boolean]
    def cleanup_or_taint
      ...
      if taint
        ct.taint_recovery!
      elsif ct.recovery_tainted?
        ct.clear_recovery_taint!
      end
      !taint
    end

    # Caller holds the container manipulation lock. This is an explicit
    # privileged acknowledgment, never part of normal callback cleanup.
    def ensure_stopped!
      state = ContainerControl::Commands::State.run!(ct, force: true)
      return if state.state == :stopped && state.init_pid.to_i <= 0

      raise InvalidNetifIdentity, 'the container has to be stopped'
    end

    def forget_host_link(name)
      ensure_stopped!

      NetInterface.sync_host_link_registry do
        netif = ct.netifs[name]
        unless netif && netif.respond_to?(:forget_absent_host_link!)
          raise InvalidNetifIdentity, 'network interface not found or not a veth'
        end

        NetInterface.validate_host_link_owner!(owner_ct: ct, owner_netif: netif)
        netif.forget_absent_host_link!
      end
    end
