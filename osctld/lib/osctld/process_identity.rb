...
module OsCtld
  ...
  class ProcessIdentity
    ...
    def initialize(pid, namespaces: [], root: false)
      ...
      @root_dir = open_proc_entry('root') if root

      namespaces.each do |name|
        ns_name = name.to_sym
        @namespaces[ns_name] = open_proc_entry(File.join('ns', ns_name.to_s))
      end
    ...
    def duplicate
      ...
      copy.send(:initialize_duplicate, self)
      copy
    end
    ...
    def open_proc_file(name, mode = 'r', &)
      io = open_proc_entry(name, flags: proc_open_flags(mode))
    ...
    def initialize_duplicate(source)
      @pid = source.pid
    ...
    def verify_root_context!
      ...
      if root_dir
        current_root = open_proc_entry('root')
      ...
      @namespaces.each do |name, held|
        current = open_proc_entry(File.join('ns', name.to_s))
    ...
    def open_proc_entry(name, **)
      @sys.openat_io(proc_dir, name, **)
    rescue SystemCallError, IOError => e
      raise Errno::ESRCH, "process #{pid} exited", e.backtrace unless alive?

      raise e.class,
            "unable to open /proc/#{pid}/#{name} through retained proc descriptor: #{e.message}",
            e.backtrace
    end
