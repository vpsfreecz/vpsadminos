...
module OsCtld
  class DistConfig::Base
    ...
    def initialize(ctrc, dcopts = {})
      @ctrc = ctrc
      @ct = ctrc.ct
      @distribution = ctrc.distribution
      @version = ctrc.version
      @dcopts = dcopts
      @configurator = configurator_class.new(ct.id, ct.distribution, ct.version)
    end
    ...
    protected
    attr_reader :dcopts, :configurator

    def with_rootfs(&block)
      if @within_rootfs
        block.call(configurator.rootfs)
      else
        ContainerControl::Commands::WithRootfs.run!(
          ctrc.ct,
          ctrc: ctrc,
          block: Proc.new do |mountpoint|
            configurator.rootfs = File.join(mountpoint, 'private')
            @within_rootfs = true
            block.call(configurator.rootfs)
          end,
          mount_ns_io: dcopts[:mount_ns_io],
          ns_pid: dcopts[:ns_pid],
          recursive: false,
        )
      end
    end
