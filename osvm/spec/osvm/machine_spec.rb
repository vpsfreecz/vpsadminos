...
RSpec.describe OsVm::Machine do
  ...
  it 'keeps the exact reaper when an immediate exit clears published state' do
    with_tmpdir do |dir|
      ...
      expect(reaper).to have_received(:join).with(1).once
  ...
  it 'keeps a sub-five-second online budget through the real shell protocol' do
    with_tmpdir do |dir|
      ...
      timeout_args = writes.first.match(/timeout --kill-after=([0-9.]+) ([0-9.]+)/)
      kill_after = timeout_args[1].to_f
      guest_timeout = timeout_args[2].to_f
      expect(kill_after).to be_between(0, 0.5).exclusive
      expect(guest_timeout).to be_between(0, 0.5).exclusive
      expect(kill_after + guest_timeout).to be < 0.5
  ...
  it 'interrupts a join when the console reports a kernel failure' do
    with_tmpdir do |dir|
      machine = build_machine(dir:)
      reaper = instance_double(OsVm::Machine::QemuReaper)
      waits = []
  ...
  it 'interrupts the post-poweroff wait on a kernel failure' do
    with_tmpdir do |dir|
      machine = build_machine(dir:)
      reaper = instance_spy(OsVm::Machine::QemuReaper, join: nil)
      allow(machine).to receive(:execute) do
