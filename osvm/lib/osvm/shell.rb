...
module OsVm
  class Shell
    OUTPUT_END_MARKER = '__OSVM_OUTPUT_END__'.freeze
    ...
    def qemu_options
      [
        ...
        # Console ports discard output under backpressure; shell replies must not.
        '-device', "virtserialport,chardev=#{chardev_id},name=org.osvm.shell#{index}"
      ]
    end
    ...
    def execute_command(cmd, timeout:, deadline: nil, timeout_message: nil)
      ...
      io.write(
        "#{workaround}#{timeout_command} bash -c #{Shellwords.escape(vm_command)} " \
        "2>&1 | (base64 -w 76; echo #{OUTPUT_END_MARKER})\n"
      )
      ...
      output = Base64.decode64(raw_output.delete_suffix("#{OUTPUT_END_MARKER}\n"))
    ...
    def read_output(timeout:, command:, deadline: nil, timeout_message: nil, terminator: "\n")
