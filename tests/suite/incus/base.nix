...
import ../../make-test.nix (
  ...
  let
    common = ''
      ...
      def incus_static_ipv4(distribution, version)
      ...
      def create_incus_container(ct, distribution, version, map_base)
        ipv4 = incus_static_ipv4(distribution, version)
        ...
        machine.all_succeed(
          ...
          # Query the bridge-local dnsmasq, which forwards through the VM resolver.
          "osctl ct set dns-resolver #{ct} 192.168.1.1",
