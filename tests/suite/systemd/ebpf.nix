    ...
    testScript = ''
      ...
      def self.dump_systemd_ebpf_startup_debug
        machine.execute(<<~SH, timeout: 180)
          ...
          cat /sys/kernel/security/apparmor/profiles 2>/dev/null | grep -E 'lxc|systemd-ebpf|ct-tank' || true
          echo '--- kernel denial diagnostics ---'
          dmesg 2>/dev/null | grep -Ei 'apparmor|audit|denied|lxc|ct-bpf|sys/fs/bpf|bpffs|bpf' | tail -n 240 || true
          ...
          grep -Eih 'systemd-ebpf|ct-tank-systemd-ebpf|lxc|apparmor|audit|denied|sys/fs/bpf|ct-bpf|bpffs|failed to start|operation not permitted|permission denied' \
          ...
          find /var/log -maxdepth 3 -type f -name current 2>/dev/null | sort | while read -r log; do
            if grep -Eiq 'systemd-ebpf|ct-tank-systemd-ebpf|lxc|apparmor|audit|denied|sys/fs/bpf|ct-bpf|bpffs|failed to start|operation not permitted|permission denied' "$log"; then
              echo "--- filtered $log ---"
              grep -Ei 'systemd-ebpf|ct-tank-systemd-ebpf|lxc|apparmor|audit|denied|sys/fs/bpf|ct-bpf|bpffs|failed to start|operation not permitted|permission denied' "$log" | tail -n 160 || true
            fi
          done
