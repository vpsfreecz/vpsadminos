  ...
  let
    ...
    systemdEbpfFdLeakProbe = pkgs.stdenv.mkDerivation {
      ...
      src = pkgs.writeText "systemd-ebpf-fd-leak-probe.c" ''
        ...
        static int load_direction_independent_skb_prog(
          enum bpf_attach_type expected_attach_type, const char *name)
        {
          struct bpf_insn insns[] = {
            INSN_MOV64_IMM(BPF_REG_0, 0),
            INSN_ALU64_IMM(BPF_ADD, BPF_REG_0, 1),
            INSN_ALU64_IMM(BPF_ADD, BPF_REG_0, 1),
            INSN_ALU64_IMM(BPF_SUB, BPF_REG_0, 1),
            INSN_ALU64_IMM(BPF_XOR, BPF_REG_0, 0),
            INSN_ALU64_IMM(BPF_OR, BPF_REG_0, 0),
            INSN_ALU64_IMM(BPF_AND, BPF_REG_0, 1),
            INSN_EXIT(),
          };
          union bpf_attr attr;
          int fd;
        ...
        static int attach_type_errno(int cgroup_fd, int prog_fd,
        ...
        static int detach_type_errno(int cgroup_fd, int prog_fd,
                                     enum bpf_attach_type attach_type)
        {
          union bpf_attr attr;
        ...
        static int id_list_contains(const __u32 *ids, __u32 count,
                                    __u32 capacity, __u32 id)
        {
          __u32 i;
        ...
        static int exercise_loopback_udp_errno(void)
        {
          ...
          recv_fd = socket(AF_INET, SOCK_DGRAM | SOCK_CLOEXEC, 0);
          if (recv_fd < 0)
            return errno;

          if (setsockopt(recv_fd, SOL_SOCKET, SO_RCVTIMEO, &timeout,
                         sizeof(timeout)) < 0) {
            err = errno;
            goto out;
          }
          ...
          send_fd = socket(AF_INET, SOCK_DGRAM | SOCK_CLOEXEC, 0);
          if (send_fd < 0) {
            err = errno;
            goto out;
          }

          if (sendto(send_fd, &sent, sizeof(sent), 0,
          ...
          if (recv(recv_fd, &received, sizeof(received), 0) !=
        ...
        static int cgroup_skb_direction_mismatch_once(
          enum bpf_attach_type expected_attach_type,
          enum bpf_attach_type attach_type, const char *label)
        {
          __u32 ids[64] = { 0 };
          __u32 post_ids[64] = { 0 };
          __u32 count = 0, post_count = 0, prog_id = 0;
          const __u32 capacity = sizeof(ids) / sizeof(ids[0]);
          char *cgroup_path;
          int cgroup_fd = -1, prog_fd = -1;
          int cgroup_open_err = 0, load_err = 0, info_err = 0;
          int attach_err = 0, query_err = 0, exercise_err = 0;
          int detach_err = 0, post_query_err = 0;
          int query_has_id = 0, post_query_has_id = 0;

          cgroup_path = current_cgroup_path();
          cgroup_fd = open(cgroup_path, O_RDONLY | O_DIRECTORY | O_CLOEXEC);
          if (cgroup_fd < 0) {
            cgroup_open_err = errno;
            goto report;
          }

          prog_fd = load_direction_independent_skb_prog(
            expected_attach_type,
            expected_attach_type == BPF_CGROUP_INET_INGRESS ?
              "sd_mis_ing" : "sd_mis_egr");
          if (prog_fd < 0) {
            load_err = -prog_fd;
            goto report;
          }
          ...
          attach_err = attach_type_errno(cgroup_fd, prog_fd, attach_type, 0);
          if (attach_err)
            goto report;

          query_err = query_attach_ids(cgroup_fd, attach_type, 0, ids,
                                       capacity, &count);
          query_has_id = query_err == 0 &&
            id_list_contains(ids, count, capacity, prog_id);

          exercise_err = exercise_loopback_udp_errno();
          detach_err = detach_type_errno(cgroup_fd, prog_fd, attach_type);
          post_query_err = query_attach_ids(cgroup_fd, attach_type, 0,
                                            post_ids, capacity, &post_count);
          post_query_has_id = post_query_err == 0 &&
            id_list_contains(post_ids, post_count, capacity, prog_id);

        report:
          printf("%s_cgroup_path=%s\n", label, cgroup_path);
          printf("%s_cgroup_open_errno=%d\n", label, cgroup_open_err);
          printf("%s_expected_attach_type=%u\n", label,
                 (unsigned int)expected_attach_type);
          printf("%s_attach_type=%u\n", label,
                 (unsigned int)attach_type);
          printf("%s_prog_load_errno=%d\n", label, load_err);
          printf("%s_prog_info_errno=%d id=%u\n", label, info_err, prog_id);
          printf("%s_attach_errno=%d flags=0\n", label, attach_err);
          printf("%s_query_errno=%d count=%u contains_id=%d\n",
                 label, query_err, count, query_has_id);
          printf("%s_exercise_errno=%d\n", label, exercise_err);
          printf("%s_detach_errno=%d\n", label, detach_err);
          printf("%s_post_query_errno=%d count=%u contains_id=%d\n",
                 label, post_query_err, post_count, post_query_has_id);

          if (prog_fd >= 0 && attach_err == 0 && detach_err != 0)
            (void)detach_type_errno(cgroup_fd, prog_fd, attach_type);
          if (prog_fd >= 0)
            close(prog_fd);
          if (cgroup_fd >= 0)
            close(cgroup_fd);
          ...
          return cgroup_open_err == 0 &&
            ...
            detach_err == 0 &&
            post_query_err == 0 &&
            !post_query_has_id ? 0 : 1;
        }

        static int cgroup_skb_direction_mismatch(void)
        {
          int ingress_to_egress_err;
          int egress_to_ingress_err;

          ingress_to_egress_err = cgroup_skb_direction_mismatch_once(
            BPF_CGROUP_INET_INGRESS, BPF_CGROUP_INET_EGRESS,
            "ingress_loaded_egress_attached");
          egress_to_ingress_err = cgroup_skb_direction_mismatch_once(
            BPF_CGROUP_INET_EGRESS, BPF_CGROUP_INET_INGRESS,
            "egress_loaded_ingress_attached");

          printf("cgroup_skb_direction_mismatch_result=%s\n",
                 ingress_to_egress_err == 0 &&
                 egress_to_ingress_err == 0 ? "pass" : "fail");

          return ingress_to_egress_err == 0 &&
            egress_to_ingress_err == 0 ? 0 : 1;
        }
        ...
        {
          return attach_type_errno(cgroup_fd, prog_fd,
        ...
        static int detach_errno(int cgroup_fd, int prog_fd)
        {
          return detach_type_errno(cgroup_fd, prog_fd,
                                   BPF_CGROUP_INET_EGRESS);
        }
        ...
        int main(int argc, char **argv)
        {
          ...
          if (strcmp(argv[1], "cgroup-skb-direction-mismatch") == 0) {
            if (argc != 2) {
              fprintf(stderr,
                      "usage: %s cgroup-skb-direction-mismatch\n",
                      argv[0]);
              return 2;
            }

            return cgroup_skb_direction_mismatch();
          }
    ...
    mkSystemdEbpfUsernsRestrictProbe =
      kernel:
      pkgs.stdenv.mkDerivation {
        ...
        systemdVmlinuxH =
          pkgs.runCommand "systemd-ebpf-userns-restrict-vmlinux-h-${kernel.modDirVersion}" { }
            ''
              mkdir -p $out/include
    ...
    mkSystemdEbpfSysctlMonitorProbe =
      kernel:
      pkgs.stdenv.mkDerivation {
        ...
        systemdVmlinuxH =
          pkgs.runCommand "systemd-ebpf-sysctl-monitor-vmlinux-h-${kernel.modDirVersion}" { }
            ''
              mkdir -p $out/include
              ${pkgs.bpftools}/bin/bpftool btf dump file ${kernel.dev}/vmlinux format c \
                > $out/include/vmlinux.h
    ...
    mkSystemdEbpfContainer =
      ...
      }:
      {
        ...
        config =
          ...
          let
            kernel = if hostKernel != null then hostKernel else config.boot.kernelPackages.kernel;
            ...
            hasSystemdNetworkdSysctlMonitor = builtins.pathExists (
              pkgs.systemd.src + "/src/network/bpf/sysctl-monitor/sysctl-monitor.bpf.c"
            );
            systemdEbpfUsernsRestrictProbe = lib.optional includeBtfSystemd (
              mkSystemdEbpfUsernsRestrictProbe kernel
            );
            systemdEbpfSysctlMonitorProbe = lib.optional hasSystemdNetworkdSysctlMonitor (
    ...
    testScript = ''
      ...
      SYSTEMD_EBPF_CTS = ${
        builtins.toJSON (
          ...
          ++ lib.optional includeBtfSystemd "systemd-ebpf-btf"
        )
      }
      HAS_SYSTEMD_NETWORKD_SYSCTL_MONITOR = ${
        if
          builtins.pathExists (pkgs.systemd.src + "/src/network/bpf/sysctl-monitor/sysctl-monitor.bpf.c")
        then
          "true"
        else
          "false"
      ...
      def self.systemd_bpf_failure_lines(journal)
        journal.lines.select do |line|
          attach_failure =
            line.match?(
              /Attaching (?:ingress|egress) BPF program .* failed/i
            )
          operation_failure =
            line.match?(/\bBPF\b/i) &&
            line.match?(/\b(?:load|attach|query|detach)\w*\b/i) &&
            line.match?(
              /(?:fail|error|Invalid argument|Operation not permitted|Permission denied)/i
            )

          attach_failure || operation_failure
        end
      end
      ...
      begin
        ...
        _, direction_mismatch_output = ct_systemd_run(
          'systemd-ebpf-direction-mismatch',
          ['IPAccounting=no'],
          [fd_leak_probe, 'cgroup-skb-direction-mismatch']
        )
        puts "=== issue 52 direction-mismatch syscall transcript ==="
        puts direction_mismatch_output
        unless direction_mismatch_output.include?(
          'cgroup_skb_direction_mismatch_result=pass'
        )
          fail(
            "cgroup-SKB direction mismatch probe did not pass:\n" \
            "#{direction_mismatch_output}"
          )
        end
        ...
        _, issue52_context = ct_sh(<<~SH)
          set -eu
          echo issue52_systemd_origin=container-installed-stock
          echo issue52_default_gate=systemd-ebpf
          echo issue52_include_btf_wrapper=#{INCLUDE_BTF_SYSTEMD ? 1 : 0}
          echo issue52_boot_id="$(cat /proc/sys/kernel/random/boot_id)"
          echo '=== systemd version and feature flags ==='
          systemctl --version
          echo '=== kernel identity ==='
          uname -a
          cat /proc/version
          echo '=== cgroup mode and namespace identities ==='
          findmnt -n -T /sys/fs/cgroup -o FSTYPE,TARGET,OPTIONS
          cat /proc/1/cgroup
          cat /proc/self/cgroup
          for name in user pid pid_for_children cgroup syslog tracing; do
            printf 'pid1_%s=' "$name"
            readlink "/proc/1/ns/$name"
            printf 'self_%s=' "$name"
            readlink "/proc/self/ns/$name"
          done
          marker="vpsadminos-issue52-cursor-$(cat /proc/sys/kernel/random/uuid)"
          printf '%s\n' "$marker" |
            systemd-cat --identifier=vpsadminos-issue52-cursor
          journalctl --sync
          journalctl -n 1 --no-pager -o export \
            SYSLOG_IDENTIFIER=vpsadminos-issue52-cursor \
            MESSAGE="$marker" >/tmp/systemd-ebpf-issue52.marker
          sed -n 's/^__CURSOR=//p' /tmp/systemd-ebpf-issue52.marker |
            tail -n 1 >/tmp/systemd-ebpf-issue52.cursor
          sed -n 's/^_BOOT_ID=//p' /tmp/systemd-ebpf-issue52.marker |
            tail -n 1 >/tmp/systemd-ebpf-issue52.boot-id
          test -s /tmp/systemd-ebpf-issue52.cursor
          test -s /tmp/systemd-ebpf-issue52.boot-id
          echo issue52_journal_cursor="$(cat /tmp/systemd-ebpf-issue52.cursor)"
          echo issue52_journal_boot_id="$(cat /tmp/systemd-ebpf-issue52.boot-id)"
        SH
        puts "=== issue 52 exact-final context ==="
        puts issue52_context
        ...
        unless accounting_ok
          _, accounting_journal = ct_sh(
            'journal_boot_id="$(cat /tmp/systemd-ebpf-issue52.boot-id)"; ' \
            'cursor="$(cat /tmp/systemd-ebpf-issue52.cursor)"; ' \
            'journalctl -b "$journal_boot_id" --after-cursor="$cursor" ' \
            '-u systemd-ebpf-ip-accounting.service --no-pager'
          )
        ...
        _, issue52_positive_journal = ct_sh(<<~SH)
          set -eu
          cursor="$(cat /tmp/systemd-ebpf-issue52.cursor)"
          journal_boot_id="$(cat /tmp/systemd-ebpf-issue52.boot-id)"
          for unit in \
            systemd-ebpf-ip-deny.service \
            systemd-ebpf-ip-allow.service \
            systemd-ebpf-ip-accounting.service
          do
            echo "=== issue 52 positive unit: $unit ==="
            systemctl show "$unit" \
            ...
            journalctl -b "$journal_boot_id" --after-cursor="$cursor" -u "$unit" \
              --no-pager -o short-precise
          done
          echo '=== issue 52 original stock-service warning matches ==='
          journalctl -b "$journal_boot_id" --after-cursor="$cursor" \
            --no-pager -o short-precise |
            grep -Ei 'Attaching (ingress|egress) BPF program .* failed' ||
            true
        SH
        puts "=== issue 52 complete positive-unit journal ==="
        puts issue52_positive_journal
        issue52_failure_lines =
          systemd_bpf_failure_lines(issue52_positive_journal)
        unless issue52_failure_lines.empty?
          fail(
            "issue 52 BPF failure diagnostics found:\n" \
            "#{issue52_failure_lines.join}"
          )
        end
        ...
        private_bpf_probe =
          [
            ...
            '--property=DelegateNamespaces=mnt',
