  ...
  { pkgs }:
  {
    ...
    testScript = ''
      ...
      def self.expect_kernel_config(name, expected)
        expected_lines =
          if expected == "n"
            ["#{name}=n", "# #{name} is not set"]
          else
            ["#{name}=#{expected}"]
          end
        grep_command = Shellwords.join(
          ["grep", "-Fx"] + expected_lines.flat_map { |line| ["-e", line] }
        )
        status, output = machine.execute(
          "timeout 30 sh -c " \
          "#{Shellwords.escape("gzip -dc /proc/config.gz | #{grep_command}")}",
          timeout: 45
        )
        expect(status).to eq(0),
          "#{expected_lines.join(" or ")} is missing from /proc/config.gz: #{output}"
      end
