...
in
{
  name = "kernel-proactive-swap-cgroups-v${version}";
    ...
    config =
      { lib, ... }:
      {
        ...
        boot.qemu.extraQemuOptions = lib.mkOverride 0 [
          "-object"
          "memory-backend-file,id=node0,size=1536M,mem-path=@OSVM_TMPDIR@/node0.mem,share=on"
          "-object"
          "memory-backend-file,id=node1,size=1536M,mem-path=@OSVM_TMPDIR@/node1.mem,share=on"
          "-numa"
  ...
  testScript = ''
          ...
          cgroup =
            ${
              if cgroupsVersion == 2 then
                ''"/sys/fs/cgroup/proactive-test"''
              else
                ''"/sys/fs/cgroup/memory/proactive-test"''
            }
          memory_usage_file =
            File.join(
              cgroup,
              cgroups_version == 2 ? "memory.current" : "memory.usage_in_bytes"
            )
          normal_swap_bytes = 64 * 1024 * 1024
          normal_swap_kb = normal_swap_bytes / 1024
          holder_bytes = 640 * 1024 * 1024
          ...
          ${
            if cgroupsVersion == 2 then
              ...
              ''
                machine.all_succeed(
                  "mkdir -p #{cgroup}",
                  "printf '%d' #{1536 * 1024 * 1024} " \
                    "> #{cgroup}/memory.limit_in_bytes",
                  "printf '%d' #{(1536 * 1024 * 1024) + normal_swap_bytes} " \
                    "> #{cgroup}/memory.memsw.limit_in_bytes"
                )
              ''
          }
          ...
          machine.wait_until_succeeds(
            "test $(cat #{memory_usage_file}) -gt #{600 * 1024 * 1024}"
          )
    ...
    PY
          ...
          ${
            if cgroupsVersion == 2 then
              ''
                machine.wait_until_succeeds(
                  "test $(cat #{cgroup}/memory.swap.current) " \
                    "-gt #{normal_swap_bytes}",
                  timeout: 180
                )
                machine.wait_until_succeeds(
                  "test $(awk '/^proactive / {print $2}' " \
                    "#{cgroup}/memory.swap.events) -gt 0",
                  timeout: 180
                )
              ''
            else
              ''
                machine.wait_until_succeeds(
                  "test $(($(cat #{cgroup}/memory.memsw.usage_in_bytes) " \
                    "- $(cat #{cgroup}/memory.usage_in_bytes))) " \
                    "-gt #{normal_swap_bytes}",
                  timeout: 180
                )
                machine.wait_until_succeeds(
                  "sh -eu -c \"echo \\$\\$ > #{cgroup}/cgroup.procs; " \
                    "test \\$(awk '\\$1 == \\\"virtual-system\\\" " \
                    "{print \\$4}' /proc/swaps) -gt 0\"",
                  timeout: 180
                )
              ''
          }
          ...
          controller_proactive_swap_bytes =
          ...
          reported_proactive_swap_kb =
            meminfo["SwapTotal"] - normal_swap_kb
          reported_proactive_swap_bytes =
            sysinfo["totalswap"] - normal_swap_bytes

          expect(controller_proactive_swap_bytes).to be > 0
          expect(reported_proactive_swap_kb).to be > 0
          expect(reported_proactive_swap_bytes).to eq(
            reported_proactive_swap_kb * 1024
          )
          expect(reported_proactive_swap_bytes).to eq(
            controller_proactive_swap_bytes
          )
          ...
          expect(virtual[2].to_i).to eq(normal_swap_kb)
          ...
          expect(virtual_system[2].to_i).to eq(reported_proactive_swap_kb)
          expect(virtual_system[3].to_i).to eq(reported_proactive_swap_kb)
