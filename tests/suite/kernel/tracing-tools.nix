  ...
  let
    ...
    btfFdLeakProbe = pkgs.stdenv.mkDerivation {
      ...
      src = pkgs.writeText "btf-fd-leak-probe.c" ''
        ...
        int main(int argc, char **argv)
        {
          int fd, local_btf_fd, staterr, fdinfo_seen, info_err, map_err;
          int local_info_err, local_map_err;
          ...
          if (local_btf_fd < 0) {
            fprintf(stderr, "container BTF load failed errno=%d\n", -local_btf_fd);
            return 1;
          }
        ...
        static void expect_nonzero_errno(const char *name, int got)
        {
          ...
          if (got == 0) {
            ...
            exit(1);
		        ...
		        int main(void)
		        {
		          int trace_fd = load_prog(BPF_PROG_TYPE_TRACEPOINT, NULL, 0);
	          ...
	          if (trace_fd < 0) {
	            fprintf(stderr, "tracepoint probe load failed errno=%d\n", -trace_fd);
	            return 1;
	          }
          printf("container_libbpf_tracepoint_probe_load=1\n");
            ...
            return 1;
	          ...
	          if (global_fd < 0) {
	            ...
	            return 1;
		          ...
		          err = load_prog(BPF_PROG_TYPE_TRACEPOINT, NULL, 1);
		          expect_errno("container_libbpf_tracepoint_nonprobe_load", -err, EPERM);
          ...
          expect_nonzero_errno(
            ...
            get_map_fd_by_id_errno(UINT32_MAX)
	          ...
	          expect_errno(
	            "container_libbpf_tracepoint_probe_test_run",
            prog_test_run_errno(trace_fd),
            EPERM
          );
          expect_errno(
            "container_libbpf_tracepoint_probe_info",
            prog_info_errno(trace_fd),
            EPERM
          );
          expect_nonzero_errno(
            "container_libbpf_tracepoint_probe_pin",
            obj_pin_errno(trace_fd)
          );
          expect_errno(
            "container_libbpf_tracepoint_probe_bind_map",
            bind_map_errno(trace_fd),
            EPERM
          );
	          expect_errno(
	            "container_libbpf_tracepoint_probe_prog_array_update",
	            prog_array_update_errno(trace_fd),
	            EPERM
	          );
	          ...
	          expect_errno(
	            "container_libbpf_tracepoint_probe_link_create",
	            link_create_errno(trace_fd),
            EPERM
          );
          expect_errno(
  ...
  in
  {
    ...
    testScript = ''
      ...
      describe 'container tracing namespace tooling', order: :defined do
        ...
        it 'lists bounded attachable bpftrace probes from the tracing namespace' do
          ct_sh_checked(<<~'SH', timeout: 300)
            set -eu
            bpftrace-list-smoke
          SH
        ...
        it 'runs useful bpftrace kprobes only for tasks in the same tracing namespace' do
          begin
            ct_sh_checked(<<~'SH', timeout: 120)
              ...
              start_probe() {
                ...
                BPFTRACE_DEBUG_OUTPUT=1 \
                  strace -f -qq -s 256 -o "$err.strace" -e trace=bpf,perf_event_open,ioctl,mmap,openat,access,read,write,newfstatat \
                  bpftrace -q -v -e "$program" \
                  >"$out" \
                  2>"$err" &
                pid=$!
                echo "$pid" >"$pidfile"
            ...
            machine.succeeds('${tcpConnectWork}/bin/hostconnect 40', timeout: 120)
            ct_sh_checked('peerconnect 40', timeout: 120, ctid: PEER_CT)
            ct_sh_checked(<<~'SH', timeout: 120)
	              set -eu
	              sleep 2
	              ...
	              if [ -s /tmp/bpftrace-scope.cross.out ]; then
            ...
            ct_sh_checked(<<~'SH', timeout: 120)
              ...
              program="$probe { printf(\"hit\\n\"); }"
              BPFTRACE_DEBUG_OUTPUT=1 \
                strace -f -qq -s 256 -o /tmp/bpftrace-scope.same.err.strace -e trace=bpf,perf_event_open,ioctl,mmap,openat,access,read,write,newfstatat \
                bpftrace -q -v -e "$program" \
        ...
        it 'runs useful bpftrace uprobes only for tasks in the same tracing namespace' do
          begin
            ct_sh_checked(<<~'SH', timeout: 120)
              ...
              program="$probe { printf(\"hit\\n\"); }"
              BPFTRACE_DEBUG_OUTPUT=1 \
                strace -f -qq -s 256 -o /tmp/bpftrace-uprobe-scope.cross.err.strace -e trace=bpf,perf_event_open,ioctl,mmap,openat,access,read,write,newfstatat \
                bpftrace -q -v -e "$program" \
              ...
              if [ -s /tmp/bpftrace-uprobe-scope.cross.out ]; then
            ...
            ct_sh_checked('tracework 2', timeout: 120, ctid: PEER_CT)
            ct_sh_checked(<<~'SH', timeout: 120)
              set -eu
              sleep 1
            ...
            ct_sh_checked(<<~'SH', timeout: 120)
              ...
              program="$probe { printf(\"hit\\n\"); }"
              BPFTRACE_DEBUG_OUTPUT=1 \
                strace -f -qq -s 256 -o /tmp/bpftrace-uprobe-scope.same.err.strace -e trace=bpf,perf_event_open,ioctl,mmap,openat,access,read,write,newfstatat \
                bpftrace -q -v -e "$program" \
        ...
        it 'rejects leaked host BTF file descriptors in the container tracing namespace' do
          ...
          ct_sh(<<~SH)
            rm -f #{Shellwords.escape(btf_leak_ct_socket)} \
              /tmp/btf-fdleak.out \
              /tmp/btf-fdleak.rc \
              /tmp/btf-fdleak.pid
            (
              btf-fd-leak-probe recv #{Shellwords.escape(btf_leak_ct_socket)} \
                >/tmp/btf-fdleak.out 2>&1
              echo $? >/tmp/btf-fdleak.rc
            ) &
            echo $! >/tmp/btf-fdleak.pid
          SH

          machine.wait_until_succeeds("test -S #{Shellwords.escape(btf_leak_host_socket)}", timeout: 30)
          machine.succeeds([btf_leak_probe, 'send', btf_leak_host_socket].shelljoin, timeout: 120)
          ct_sh("timeout 30 sh -c 'until test -s /tmp/btf-fdleak.rc; do sleep 0.1; done'")
          _, btf_leak_output = ct_sh(
            'cat /tmp/btf-fdleak.out; test "$(cat /tmp/btf-fdleak.rc)" = 0'
          )
        ...
        it 'rejects leaked host perf event file descriptors in the container tracing namespace' do
          ...
          ct_sh(<<~SH)
            rm -f #{Shellwords.escape(perf_leak_ct_socket)} \
              /tmp/perf-fdleak.out \
              /tmp/perf-fdleak.rc \
              /tmp/perf-fdleak.pid
            (
              perf-fd-leak-probe recv #{Shellwords.escape(perf_leak_ct_socket)} \
                >/tmp/perf-fdleak.out 2>&1
              echo $? >/tmp/perf-fdleak.rc
            ) &
            echo $! >/tmp/perf-fdleak.pid
          SH

          machine.wait_until_succeeds("test -S #{Shellwords.escape(perf_leak_host_socket)}", timeout: 30)
          machine.succeeds([perf_leak_probe, 'send', perf_leak_host_socket].shelljoin, timeout: 120)
          ct_sh("timeout 30 sh -c 'until test -s /tmp/perf-fdleak.rc; do sleep 0.1; done'")
          _, perf_leak_output = ct_sh(
            'cat /tmp/perf-fdleak.out; test "$(cat /tmp/perf-fdleak.rc)" = 0'
          )
