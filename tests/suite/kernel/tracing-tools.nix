    ...
    perfFdLeakProbe = pkgs.stdenv.mkDerivation {
      ...
      src = pkgs.writeText "perf-fd-leak-probe.c" ''
        ...
        static int send_fd_over_socket(const char *socket_path, int fd)
        {
          ...
          /* Keep the event's task alive until all receiver checks finish. */
          if (recv(sock, &data, sizeof(data), MSG_WAITALL) != sizeof(data) ||
              data != 'D') {
            fprintf(stderr, "perf receiver did not acknowledge completion\n");
            exit(1);
          }
        ...
        static int recv_fd_over_socket(const char *socket_path, int *sender)
        {
          struct sockaddr_un addr;
          struct msghdr msg;
          struct cmsghdr *cmsg;
        ...
        int main(int argc, char **argv)
        {
          ...
          fd = recv_fd_over_socket(argv[2], &sender);
          ...
          close(fd);
          if (send(sender, "D", 1, MSG_NOSIGNAL) != 1)
            die_errno("acknowledge perf sender");
          close(sender);
    ...
    testScript = ''
      ...
      def self.start_host_exec_marker_loop
        machine.succeeds(<<~'SH')
          ...
          # A renamed coreutils multi-call symlink dispatches by argv[0] and fails.
          # Execute a real script at the exact path observed by the tracepoint.
          printf '#!/bin/sh\nexit 0\n' > "$marker"
          chmod 0755 "$marker"
          "$marker"
          (
        ...
        it 'runs useful bpftrace kprobes only for tasks in the same tracing namespace' do
          begin
            ct_sh_checked(<<~'SH', timeout: 120)
              ...
              wait_probe_ready() {
                ...
                while [ "$attempt" -lt 300 ]; do
                  if grep -Fxq '__BPFTRACE_NOTIFY_PROBES_ATTACHED' "$out"; then
                    return 0
                  fi

                  state=$(ps -o stat= -p "$(cat "$pidfile")" 2>/dev/null | awk '{print $1}')
                  case "$state" in
                    ""|Z*)
                      echo "bpftrace probe did not stay alive while attaching $probe" >&2
                ...
                echo "bpftrace probe did not attach within 60 seconds: $probe" >&2
                cat "$out" >&2 2>/dev/null || true
                cat "$err" >&2 2>/dev/null || true
            ...
            ct_sh_checked(<<~'SH', timeout: 120)
              ...
              out=/tmp/bpftrace-scope.same.out
              err=/tmp/bpftrace-scope.same.err
              pidfile=/tmp/bpftrace-scope.same.pid
              ...
              while [ "$attempt" -lt 300 ]; do
                ...
                case "$state" in
                  ""|Z*)
                    echo "bpftrace same-namespace probe did not stay alive while attaching" >&2
                    cat "$out" >&2 2>/dev/null || true
                    cat "$err" >&2 2>/dev/null || true
                    wait "$(cat "$pidfile")" 2>/dev/null || true
                    exit 1
              ...
              if ! grep -Fxq '__BPFTRACE_NOTIFY_PROBES_ATTACHED' "$out"; then
                echo "bpftrace same-namespace probe did not attach within 60 seconds" >&2
                cat "$out" >&2 2>/dev/null || true
                cat "$err" >&2 2>/dev/null || true
                kill -INT "$(cat "$pidfile")" 2>/dev/null || true
                wait "$(cat "$pidfile")" 2>/dev/null || true
              ...
              if grep -qx 'hit' "$out"; then
                echo "bpftrace same-namespace probe produced hits before workload" >&2
                cat "$out" >&2
                cat "$err" >&2 2>/dev/null || true
                exit 1
              fi
        ...
        it 'runs useful bpftrace uprobes only for tasks in the same tracing namespace' do
          begin
            ct_sh_checked(<<~'SH', timeout: 120)
              ...
              out=/tmp/bpftrace-uprobe-scope.cross.out
              err=/tmp/bpftrace-uprobe-scope.cross.err
              pidfile=/tmp/bpftrace-uprobe-scope.cross.pid
              ...
              while [ "$attempt" -lt 300 ]; do
                ...
                case "$state" in
                  ""|Z*)
                    echo "bpftrace uprobe did not stay alive while attaching" >&2
                    cat "$out" >&2 2>/dev/null || true
                    cat "$err" >&2 2>/dev/null || true
                    wait "$(cat "$pidfile")" 2>/dev/null || true
                    exit 1
              ...
              if ! grep -Fxq '__BPFTRACE_NOTIFY_PROBES_ATTACHED' "$out"; then
                echo "bpftrace uprobe did not attach within 60 seconds" >&2
                cat "$out" >&2 2>/dev/null || true
                cat "$err" >&2 2>/dev/null || true
                kill -INT "$(cat "$pidfile")" 2>/dev/null || true
                wait "$(cat "$pidfile")" 2>/dev/null || true
            ...
            ct_sh_checked(<<~'SH', timeout: 120)
              ...
              out=/tmp/bpftrace-uprobe-scope.same.out
              err=/tmp/bpftrace-uprobe-scope.same.err
              pidfile=/tmp/bpftrace-uprobe-scope.same.pid
              ...
              while [ "$attempt" -lt 300 ]; do
                ...
                case "$state" in
                  ""|Z*)
                    echo "bpftrace same-namespace uprobe did not stay alive while attaching" >&2
                    cat "$out" >&2 2>/dev/null || true
                    cat "$err" >&2 2>/dev/null || true
                    wait "$(cat "$pidfile")" 2>/dev/null || true
                    exit 1
              ...
              if ! grep -Fxq '__BPFTRACE_NOTIFY_PROBES_ATTACHED' "$out"; then
                echo "bpftrace same-namespace uprobe did not attach within 60 seconds" >&2
                cat "$out" >&2 2>/dev/null || true
                cat "$err" >&2 2>/dev/null || true
                kill -INT "$(cat "$pidfile")" 2>/dev/null || true
                wait "$(cat "$pidfile")" 2>/dev/null || true
              ...
              if grep -qx 'hit' "$out"; then
                echo "bpftrace same-namespace uprobe produced hits before workload" >&2
                cat "$out" >&2
                cat "$err" >&2 2>/dev/null || true
                exit 1
              fi
