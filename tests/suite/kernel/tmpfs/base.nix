...
import ../../../make-test.nix ({ pkgs }: {
  name = "kernel-tmpfs-cgroups-v${toString cgroupsVersion}";

  description = ''
    Test tmpfs UID/GID and size within containers with cgroups v${toString cgroupsVersion}

    UID/GID from the container's point of view is mapped to its user namespace,
    from the host we expect to see unmapped values.

    Default tmpfs size must respect the configured memory limit instead of the total
  ...
  testScript = ''
    ...
    host_root_uid = rand(100_000..900_000)
    host_root_gid = rand(100_000..900_000)
    ...
    limits.each do |memory_limit, swap_limit|
      ...
      describe "tmpfs in container with #{limits_str}" do
...

        {
          ...
          context variant do
            after(:context) do
              machine.succeeds("osctl ct exec #{testct} umount /mnt/tmpfs")
              machine.succeeds("osctl ct exec #{testct} rmdir /mnt/tmpfs")
            end
            ...
            it 'can be mounted' do
              ...
              machine.all_succeed(
                "osctl ct exec #{testct} mkdir /mnt/tmpfs",
