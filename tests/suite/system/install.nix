            ...
            let
              ...
              rootTests = (kernelPackages.genZfsUserPackage config.boot.kernelVersion).overrideAttrs (old: {
                postInstall = lib.replaceStrings [ "rm -rf $out/share/zfs/zfs-tests" ] [ "" ] old.postInstall + ''
                  ...
                  # findmnt renders a bind mount sourced from a dataset as
                  # pool/dataset[/subdir], and only the dataset name is a valid
                  # argument to zfs mount, so normalise and deduplicate the
                  # collected names. The body's assertions are untouched.
                  substituteInPlace $out/share/zfs/zfs-tests/tests/functional/rootpool/rootpool_002_neg.ksh \
                    --replace-fail ${lib.escapeShellArg ''mount -p | awk '$4 == "zfs" {print $1}' ''} \
                      'findmnt -rn -t zfs -o SOURCE | sed "s/\[.*$//" | sort -u '
                '';
              });
            in
            {
    ...
    testScript = ''
      ...
      describe 'vpsAdminOS installation', order: :defined do
        ...
        ${pkgs.lib.optionalString zfsRoot ''
          it 'executes original ZFS rootpool bodies on the mounted installed root' do
            installed.all_succeed(
              ...
              # A fresh installation has no /var/tmp; the original bodies use
              # it as TEST_BASE_DIR for their repair inventory.
              'mkdir -p /var/tmp'
            )
            # rootpool_003_neg is deliberately not run: it requires the kernel
            # to refuse renaming the dataset that hosts the running root, which
            # Linux allows. Upstream disables this whole group on Linux and
            # FreeBSD because it manipulates the system root pool
            # (tests/functional/rootpool/setup.ksh: "Supported but disabled by
            # default"). The observed Linux behaviour and the disposition are
            # recorded in the release-readiness tracker.
            %w[rootpool_002_neg rootpool_007_pos].each do |body|
        ...
        it 'boots the selected generation and imports/autostarts persistent state again' do
          ...
          # The installed system has no service manager handling a plain
          # poweroff request; force it exactly as the installer teardown does.
          installed.succeeds('poweroff -f')
