import ../../make-test.nix (
    ...
    reproducer = pkgs.writeScript "fallocate-deadlock-reproducer.py" ''
      ...
      def main():
          ...
          for attempt in range(1, ATTEMPTS + 1):
              ...
              if outcome not in (OUTCOME_COMPLETED, OUTCOME_EARLY_EXIT):
                  log_attempt_summary(results)
                  return 2 if outcome == OUTCOME_REPRODUCED else 3
  ...
  in
  {
    ...
    testScript = ''
      ...
      machine.succeeds('echo 8 4 1 7 > /proc/sys/kernel/printk')
      ...
      begin
        ...
        # Retain diagnostics even if deadlocked children prevent command exit.
        status, output = machine.execute(
          "#{command} 2>&1 | tee /run/osvm/shared-dir/fallocate-reproducer.log",
          timeout: 40 * 60
        )
      ensure
