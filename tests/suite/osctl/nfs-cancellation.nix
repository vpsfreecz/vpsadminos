import ../../make-template.nix (
  ...
  {
    instance = kernelVersion + (if diagnostic then "-debug" else "");
    test =
      { pkgs }:
      let
        dirtyInitProgram = pkgs.pkgsStatic.stdenv.mkDerivation {
          ...
          buildPhase = ''
            ...
            "$CC" -std=gnu11 -O2 -Wall -Wextra -Werror -o sysfs-race sysfs-race.c
          '';
          installPhase = ''
            install -Dm755 dirty-init "$out/bin/dirty-init"
            install -Dm755 sysfs-race "$out/bin/sysfs-race"
          '';
        };
        dirtyInit = pkgs.writeScript "nfs-dirty-init.sh" ''
          ...
          exec /sbin/nfs-dirty-init-program "/mnt/nfs/dirty-init-$1" "''${2:-}"
        '';
      in
      {
        ...
        machine = import ../../machines/vpsadminos/with-tank.nix {
          inherit pkgs;
          config = {
            imports = pkgs.lib.optional diagnostic ../../../os/configs/nfs-cancellation-debug.nix;
        ...
        testScript = ''
          ...
          before(:suite) do
            ...
            machine.succeeds("test -e /sys/fs/nfs/net/nfs_client/shutdown_tree || modprobe nfs")
          ...
          def stop_nfs_client(options = '--kill', timeout: 70)
          ...
          ${pkgs.lib.optionalString diagnostic ''
            describe 'sysfs lifetime under fault injection', order: :defined do
              %w[3 4.2].each do |version|
                it "drains concurrent sysfs writes during NFS #{version} mount failure and unmount" do
                  init_pid = Integer(machine.succeeds('osctl ct show -H -o init_pid nfs1')[1].strip)
                  fault = '/sys/kernel/debug/fail_function'
                  machine.all_succeed(
                    "test -d #{fault}",
                    "grep -w nfs_init_server_rpcclient #{fault}/injectable",
                    'rm -f /run/nfs-race-stop /run/nfs-race-result',
                    "nsenter -t #{init_pid} --net unshare --mount sh -c " \
                      "'mount --make-rslave /; mount -t sysfs sysfs /sys; " \
                      "exec ${dirtyInitProgram}/bin/sysfs-race /run/nfs-race-stop' " \
                      '>/run/nfs-race-result 2>/run/nfs-race-error </dev/null &',
                    "echo nfs_init_server_rpcclient > #{fault}/inject",
                    "printf '%#x' -12 > #{fault}/nfs_init_server_rpcclient/retval",
                    "echo 1000 > #{fault}/times",
                    "echo 0 > #{fault}/verbose",
                    "echo 100 > #{fault}/probability",
                  )
                  begin
                    # Every injected failure occurs after server-N is published.
                    # The writer must observe EAGAIN while that server is unready.
                    machine.succeeds(
                      "osctl ct exec nfs1 sh -c 'mkdir -p /mnt/nfs-race; " \
                        "for i in $(seq 1 300); do " \
                        "if mount -t nfs -o vers=#{version},proto=tcp,nolock " \
                        "10.0.0.10:/srv/nfs-cancellation /mnt/nfs-race; then exit 1; fi; done'",
                      timeout: 600,
                    )
                    expect(machine.succeeds("cat #{fault}/times")[1].to_i).to be < 1000
                    machine.succeeds("echo 0 > #{fault}/probability")
                    machine.succeeds(
                      "osctl ct exec nfs1 sh -c 'for i in $(seq 1 100); do " \
                        "mount -t nfs -o vers=#{version},proto=tcp,nolock " \
                        "10.0.0.10:/srv/nfs-cancellation /mnt/nfs-race && " \
                        "umount /mnt/nfs-race; done; true'",
                      timeout: 600,
                    )
                  ensure
                    machine.all_succeed(
                      "echo 0 > #{fault}/probability",
                      "echo '!nfs_init_server_rpcclient' > #{fault}/inject",
                      'touch /run/nfs-race-stop',
                    )
                  end
                  machine.wait_until_succeeds('test -s /run/nfs-race-result', timeout: 60)
                  result = JSON.parse(machine.succeeds('cat /run/nfs-race-result')[1])
                  expect(result.fetch('unready')).to be > 0
                  expect(result.fetch('writes')).to be > 0
                  expect(result.fetch('errors')).to eq(0)
                  # Wait beyond DEBUG_KOBJECT_RELEASE's delayed cleanup window.
                  sleep(10)
                  mount_nfs('nfs1', version)
                  machine.succeeds('osctl ct exec nfs1 sh -c "echo after-race > /mnt/nfs/after-race; sync"')
                  expect(machine.succeeds('cat /srv/nfs-cancellation/after-race')[1].strip).to eq('after-race')
          ...
          ${pkgs.lib.optionalString (!diagnostic) ''
            %w[3 4.0 4.1 4.2].each do |version|
              describe "NFS #{version}", order: :defined do
                ...
                it "restores cancellation handles after osctld restarts" do
                  isolate_client('I')
                  begin
                    start_writer(version)
                    sleep(5)
                ...
                [false, true].each do |restart_daemon|
                it "finishes dirty PID1 exit (restore original namespaces: #{restart_daemon})" do
                  machine.all_succeed(
                  ...
                  mode = restart_daemon ? 'detach' : ""
                  machine.succeeds("osctl ct set init-cmd nfs1 /sbin/nfs-dirty-init #{version} #{mode}")
                  begin
                    machine.succeeds("osctl ct start nfs1", timeout: 60)
                    ...
                    begin
                      ...
                      if restart_daemon
                        # Init has entered a descendant user/net namespace. Stop
                        # osctld before exit so only the durable original pins can
                        # recover the NFS client after the daemon starts again.
                        machine.succeeds("sv -w 60 down osctld", timeout: 90)
                      end
                      machine.succeeds("printf e > #{rootfs}/root/nfs-init-control")
                      if restart_daemon
                        machine.wait_until_succeeds(
                          "ps -eo stat=,comm= | grep -E '^D.*nfs-dirty-init'",
                          timeout: 30,
                        )
                        machine.succeeds("sv up osctld")
                        machine.wait_for_service('osctld')
                        machine.wait_for_osctl_pool('tank')
                      end
                      machine.wait_until_succeeds(
                      ...
                      machine.succeeds("sv up osctld") if restart_daemon
                      isolate_client('D')
                    end
                  ensure
          ...
          after(:suite) do
            output = machine.succeeds('dmesg')[1]
            expect(output).not_to match(/BUG:|kernel BUG at|WARNING:|Oops:|general protection fault|[Kk]ernel panic|possible circular locking|inconsistent lock state|KASAN:/)
          end
        '';
      };
  }
