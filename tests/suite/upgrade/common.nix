      ...
      testScript = ''
        ...
        marker = saved.fetch(:impermanent) ? '/persistent/upgrade-retained' : '/root/upgrade-retained'
                ...
                ${
                  if overlapActivation then
                    ''
                      ...
                      machine.write_file('/root/overlap-client.rb', <<~CLIENT)
                        require 'socket'
                        require 'json'
                        ...
                        begin
                          ...
                          abort 'partial request unexpectedly received a response' unless socket.recv(1024).empty?
                      ...
                      CLIENT
                      machine.succeeds("(${pkgs.ruby}/bin/ruby /root/overlap-client.rb; echo $? > /overlap-client.status) > /overlap-client.log 2>&1 < /dev/null & echo $! > /overlap-client.pid")
                      overlap_script = "#!/bin/sh\necho started > /overlap-runscript.started\nsleep 60\necho overlap-runscript-done > /overlap-runscript\n"
                      machine.succeeds("(osctl ct exec upgrade1 sh -c 'echo started > /overlap-exec.started; sleep 60; echo overlap-exec-done > /overlap-exec'; echo $? > /overlap-exec.status) > /overlap-exec.log 2>&1 < /dev/null & echo $! > /overlap-exec.pid")
                      machine.succeeds("(printf %s #{Shellwords.escape(overlap_script)} | osctl ct runscript upgrade2 -; echo $? > /overlap-runscript.status) > /overlap-runscript.log 2>&1 < /dev/null & echo $! > /overlap-runscript.pid")
                ...
                ${
                  if transferAcrossActivation then
                    ''
                      machine.succeeds('osctl ct import --as-id transfernew /root/upgrade-transfer.tar', timeout: 300)
                      ...
                      machine.all_succeed(
                        ...
                        "${pkgs.acl}/bin/getfacl -p #{transfer_root}/upgrade-transfer/acl | grep -E '^user:4321:rwx$'",
