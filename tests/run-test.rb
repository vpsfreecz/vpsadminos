...
class Machine
  ...
  def execute(cmd, timeout: TIMEOUT)
    fail "Machine not running" unless running?
...
class Test
  ...
  def tmpdir
    File.join(ENV['TMPDIR'] || '/tmp', "os-test-#{config[:name]}")
  end
end
...
class Cli
  ...
  def run
    parser = OptionParser.new do |opts|
      opts.banner = "Usage: #{$0} [options] <test-config>"

      opts.on('--[no-]destcructive', 'Determines whether disk files are destroyed') do |v|
        options[:destructive] = v
      end
    end
    parser.parse!(argv)
